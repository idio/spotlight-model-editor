...
`knife ec2 server create -r "role[spotlight]" -I ami-6d3f9704 -G default -x ubuntu --node-name "dev-spotlight" --environment "development" -f m3.xlarge --availability-zone us-east-1d`
...
The command would -rexport the model, so you can just zip and upload the file to S3 to be used. Then delete the server.
